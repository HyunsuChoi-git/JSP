...
public class BoardDAO {
	...
	//검색 글 갯수 돌려주는 메소드
	public int getArticleCount(String sel, String select) {
		int x = 0;
		try {
			conn = getConnection();
			String sql = "select count(*) from board where "+sel+" like '%"+select+"%'";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				x = rs.getInt(1);
	...
	// 검색 게시글들 리턴해주는 메서드
	public List getArticles(String sel, String select, int start, int end) {
		List articleList = null;
		try {
			conn = getConnection();
			String sql = "select num,writer,subject,email,content,pw,reg,readcount,ip,ref,re_step,re_level,r "
					+ "from (select num,writer,subject,email,content,pw,reg,readcount,ip,ref,re_step,re_level,rownum r "
					+ "from (select num,writer,subject,email,content,pw,reg,readcount,ip,ref,re_step,re_level "
					+ "from board where "+sel+" like '%"+select+"%' order by ref desc, re_step asc) order by ref desc, re_step asc) "
					+ "where r >= ? and r <= ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, start);
			pstmt.setInt(2, end);
